odoo.define("odex25_documents_spreadsheet.PivotController", function (require) {
    "use strict";
    ...
    const SpreadsheetSelectorDialog = require("odex25_documents_spreadsheet.SpreadsheetSelectorDialog");
    const pivotUtils = require("odex25_documents_spreadsheet.pivot_utils");
    const spreadsheet = require("odex25_documents_spreadsheet.spreadsheet_extended");
    ...
    PivotController.include({
        ...
        //     const ready = session.user_has_group("dms.group_documents_user").then((has_group) => {
        //         this.canInsertPivot = has_group;
        //     });

        //     return Promise.all([sup, ready]);
        // },

        willStart() {
            const sup = this._super(...arguments);
            const acl = session.user_has_group('dms.group_documents_user').then(hasGroup => {
                this.canInsertPivot = hasGroup;
            });
        ...
        _initializeModel(workbookData) {
            ...
            if (!isNewModel) {
                // activate the last sheet to insert the pivot sheet at the end.
                const sheets = model.getters.getSheets();
                model.dispatch("ACTIVATE_SHEET", {
                    from: model.getters.getActiveSheet(),
                    to: sheets[sheets.length - 1].id,
                });

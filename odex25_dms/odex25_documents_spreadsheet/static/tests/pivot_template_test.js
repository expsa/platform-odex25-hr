odoo.define("odex25_documents_spreadsheet.spreadsheet_template_tests", function (require) {
    "use strict";

    const pivotUtils = require("odex25_documents_spreadsheet.pivot_utils");
    const { jsonToBase64, base64ToJson } = pivotUtils;
    const spreadsheetUtils = require("odex25_documents_spreadsheet.test_utils");
    const testUtils = require("web.test_utils");
    const DocumentsKanbanView = require("odex25_documents_spreadsheet.KanbanView");
    const DocumentsListView = require("odex25_documents_spreadsheet.ListView");
    const TemplateListView = require("odex25_documents_spreadsheet.TemplateListView");
                ...
                const [actionManager, model, env] = await createSpreadsheetFromPivot({
                    ...
                    intercepts: {
                        do_action: function (ev) {
                            ...
                            assert.equal(
                                ev.data.action,
                                "odex25_documents_spreadsheet.save_spreadsheet_template_action"
                            );

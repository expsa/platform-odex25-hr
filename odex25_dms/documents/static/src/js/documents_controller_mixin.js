...
const DocumentsControllerMixin = Object.assign({}, fileUploadMixin, {
    ...
    events: {
        ...
        'click .o_list_export_pdf': '_onClickDocumentsPdfFromUrl',
     ...
     */
    renderButtons($node) {
        this.$buttons = $(qweb.render('DocumentsViews.buttons', {'viewtype': this.viewType}));
        if ($node) {
    ...
    _onClickDocumentsRequest(ev) {
        ev.preventDefault();
        const context = this.model.get(this.handle, {raw: true}).getContext();
        this.do_action('dms.action_request_form', {
            additional_context: {
    ...
    _onClickDocumentsUploadFromUrl(ev) {
        ev.preventDefault();
        const context = this.model.get(this.handle, {raw: true}).getContext();
        this.do_action('dms.action_url_form', {
            additional_context: {
                default_partner_id: context.default_partner_id || false,
                default_folder_id: this.searchModel.get('selectedFolderId'),
                default_tag_ids: [[6, 0, this.searchModel.get('selectedTagIds')]],
    ...
    _onClickDocumentsPdfFromUrl(ev) {
        ...
        'domain':this._getExportDialogWidget().record.domain
            ...
            if (groupedBy[0]){
                ...
                var action = {
                            ...
                            'report_name':'documents.export_in_pdf_group_by',
                ...
                var action = {
                        'type': 'ir.actions.report',
                        'report_type': 'qweb-pdf',
                        'report_name':'documents.export_in_pdf',

...
const DocumentsInspector = Widget.extend({
    ...
    init: function (parent, params) {
        ...
        this._permission_write = this.records.some(record =>
             record.data.permission_write === true
        );
        this._perm_download = this.records.some(record =>
             record.data.perm_download === true
        );
    ...
    async start() {
        ...
        this.$('.o_inspector_table .o_input').prop('disabled', this._isLocked || !this._permission_write );
//        this.$('.o_inspector_table .o_input').prop('disabled', this._permission_write );
    },
    ...
    _renderFields: function () {
        ...
        if (this.records.length === 1) {
            ...
            proms.push(this._renderField('partner_id', {
                icon: 'fa fa-user',
                mode: 'edit',
            }));
            proms.push(this._renderField('file_size', {
                icon: 'fa fa-compress',
            }));
            proms.push(this._renderField('file_code', {
                icon: 'fa fa-qrcode',
                mode: 'edit',
            }));
            proms.push(this._renderField('key_words', options));
            proms.push(this._renderField('category_id', {
                icon: 'fa fa-sitemap',
                mode: 'edit',
            }));
            proms.push(this._renderField('version', {
                icon: 'fa fa-files-o',
                mode: 'edit',
            }));
        ...
        if (this.records.length > 0) {
            proms.push(this._renderField('project_id', {
                icon: 'fa fa-desktop',
                mode: 'edit',
            }));
            proms.push(this._renderField('owner_id', {
                icon: 'fa fa-user o_documents_folder_color',
                mode: 'edit',
            }));
    ...
    _renderTags: function () {
        ...
        if (this.tags.length > this.commonTagIds.length) {
            ...
            const disabled = this._isLocked || !this._permission_write || (this.records.length === 1 && !this.records[0].data.active);
    ...
    _updateButtons: function () {
        ...
        if (this._isLocked || !this._permission_write) {
        ...
        if (!this._perm_download || !binary && (this.records.length > 1 || (this.records.length && this.records[0].data.type === 'empty'))) {

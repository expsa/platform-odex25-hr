...
class DmsDirectory(models.Model):
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        if field_name == 'parent_id':
            ...
            folder_domain = expression.OR(
                [[('parent_id', 'child_of', available_folders.ids)], [('id', 'in', available_folders.ids)]])
    ...
    @api.depends("child_directory_ids", "file_ids", 'child_directory_ids')
    def _compute_count_elements(self):
        for record in self:
            record.count_total_files = len(record.file_ids)
            record.count_total_directories = len(record.child_directory_ids)
            print(record.count_files)
    ...
    #     for rec in self:
    #         rec.count_total_files = len(rec.directory_ids)

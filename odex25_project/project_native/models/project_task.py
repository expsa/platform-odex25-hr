...
class ProjectTaskPredecessor(models.Model):
    ...
    def create(self, vals):
        self.check_parent(vals["parent_task_id"])
        result = super(ProjectTaskPredecessor, self).create(vals)

        if result.task_id.schedule_mode == "manual":
            second_between = (result.task_id.date_end - result.task_id.date_start).total_seconds()
            if result.type == "FS":
                result.task_id.date_start = result.parent_task_id.date_end
                result.task_id.date_end = result.task_id.date_start + timedelta(seconds=second_between)

            elif result.type == "SS":
                result.task_id.date_start = result.parent_task_id.date_start
                result.task_id.date_end = result.task_id.date_start + timedelta(seconds=second_between)

            elif result.type == "FF":
                result.task_id.date_end = result.parent_task_id.date_end
                result.task_id.date_start = result.task_id.date_end - timedelta(seconds=second_between)

            elif result.type == "SF":
                result.task_id.date_end = result.parent_task_id.date_start
                result.task_id.date_start = result.task_id.date_end - timedelta(seconds=second_between)

...
class ProjectTaskCriticalPath(models.Model):
    ...
    critical_path = fields.Boolean(name="is Critical Path", help="is Critical Path", default=False, readonly=True)
    # critical path
    ...
    def _critical_path_calc(self, task):
        ...
        if all(key in task for key in need_key):
            if task["late_date_start"] and task["soon_date_start"]:
                ...
                soon_date_start = task["soon_date_start"]
                late_date_start = task["late_date_start"]
            ...
            if task["late_date_end"] and task["soon_date_end"]:
                ...
                soon_date_end = task["soon_date_end"]
                late_date_end = task["late_date_end"]

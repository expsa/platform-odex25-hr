odoo.define('odex25_account_reports.account_report', function (require) {
'use strict';
...
var accountReportsWidget = AbstractAction.extend({
    ...
    filter_accounts: function(e) {
        ...
        this.$('.o_account_searchable_line').each(function(index, el) {
            ...
            var lineText = $accountReportLineFoldable.find('.account_report_line_name').contents().get(0);
                // Only the direct text node, not text situated in other child nodes
            if (lineText) {
                lineText = lineText.nodeValue.trim();
            }
            else {
                lineText = '';
            }

...
class generic_tax_report(models.AbstractModel):
    ...
    def _generate_tax_closing_entry(self, options, move=False, raise_on_empty=False):
        ...
        on_empty_action = self.env.ref('odex25_account_accountant.action_tax_group')
    ...
    def _get_templates(self):
        ...
        rslt['search_template'] = 'odex25_account_reports.search_template_generic_tax_report'
        return rslt
    ...
    def _get_grids_refund_sql_condition(self):
        ...
        This function is for example overridden in pos_odex25_account_reports.
        """
    ...
    def _get_lines_by_tax(self, options, line_id, taxes):
        ...
        for tax_account in taxes.values():
            for account_id, tax in tax_account.items():
                # Check if tax is a dictionary and contains 'obj'
                if not isinstance(tax, dict) or 'obj' not in tax:
                    print("Error: 'obj' key is missing or tax is not a dictionary for account_id:", account_id)
                    continue
                ...
                if tax['obj'].amount_type == 'group':
                    ...
                    # - If at least one child is 'none', show the group of taxes.
                    # - If all children are different from 'none', only show the children.
                    tax['children'] = []
                    tax['show'] = False
                    for child in tax['obj'].children_tax_ids:
                        if child.type_tax_use != 'none':

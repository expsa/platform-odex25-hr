...
class AccountCashFlowReport(models.AbstractModel):
    ...
    def get_liquidity_move_from_periods(self, options, period, currency_table_query, status):
        current_options_period = options.copy()
        current_options_period['date'] = {
            'mode': 'single',
            'date_from': period['date_from'],
            'date_to': period['date_to'],
        }
        payment_move_ids, payment_account_ids = self._get_liquidity_move_ids(current_options_period)
        if status == 'liquidity':
            res = self._get_liquidity_move_report_lines(current_options_period, currency_table_query, payment_move_ids,
                                                        payment_account_ids)
        else:
            res = self._get_reconciled_move_report_lines(current_options_period, currency_table_query, payment_move_ids,
                                                         payment_account_ids)
        return res
    ...
    @api.model
    def _compute_liquidity_balance_only(self, options, currency_table_query, account_id):
    ...
    def get_balance_from_periods(self, options, currency_table_query, account_id):
        balance = []
        for period in options['comparison'].get('periods'):
            ...
            result = self._compute_liquidity_balance_only(current_options_period, currency_table_query, account_id)
            if result:
                balance.append(result[0])
            else:
                balance.append(0.0)
        return balance
    ...
    #     columns_header = [{'name': ''}, {'name': _('Balance'), 'class': 'number'}]
    #     if options.get('comparison') and options['comparison'].get('periods'):
    #         for p in options['comparison']['periods']:
    #             columns_header += [{'name': '%s \n %s' % (_('Balance'), p.get('string')), 'class': 'number',
    ...
    def _get_columns(self, options):
        header1 = [
                      ...
                      {'name': period['string'], 'class': 'number'}
                      for period in options['comparison'].get('periods', [])
                  ]
        return [header1]
        ...
        return [
            {
                ...
                'columns': [{'name': 0.0, 'class': 'number'} for k in range(0, (period_number + 1))],
    ...
    def _get_lines(self, options, line_id=None):
        ...
        def _insert_at_index(index, account_id, account_code, account_name, amount):
            ...
            for value in amount:
                if value:
                    line['columns'][i]['name'] += round(value, 2)
                    line['unfolded_lines'][account_id]['columns'][i]['name'] += round(value, 2)
                i += 1
        ...
        def _dispatch_result(account_id, account_code, account_name, account_internal_type, amount):
            ...
            if account_internal_type == 'receivable':
                ...
                _insert_at_index(3, account_id, account_code, account_name, [-a for a in amount])
            elif account_internal_type == 'payable':
                # 'Advance Payments made to suppliers'                      (index=5)
                _insert_at_index(5, account_id, account_code, account_name, [-a for a in amount])
            # else:
            # for amount in amounts:
            elif any([a < 0 for a in amount]):
                if tag_operating_id in tags_per_account.get(account_id, []):
                    # 'Cash received from operating activities'             (index=4)
                    _insert_at_index(4, account_id, account_code, account_name, [-a for a in amount])
                elif tag_investing_id in tags_per_account.get(account_id, []):
                    # 'Cash in for investing activities'                    (index=8)
                    _insert_at_index(8, account_id, account_code, account_name, [-a for a in amount])
                elif tag_financing_id in tags_per_account.get(account_id, []):
                    # 'Cash in for financing activities'                    (index=11)
                    _insert_at_index(11, account_id, account_code, account_name, [-a for a in amount])
                else:
                    # 'Cash in for unclassified activities'                 (index=14)
                    _insert_at_index(14, account_id, account_code, account_name, [-a for a in amount])
            elif any([a > 0 for a in amount]):
                if tag_operating_id in tags_per_account.get(account_id, []):
                    # 'Cash paid for operating activities'                  (index=6)
                    _insert_at_index(6, account_id, account_code, account_name, [-a for a in amount])
                elif tag_investing_id in tags_per_account.get(account_id, []):
                    # 'Cash out for investing activities'                   (index=9)
                    _insert_at_index(9, account_id, account_code, account_name, [-a for a in amount])
                elif tag_financing_id in tags_per_account.get(account_id, []):
                    # 'Cash out for financing activities'                   (index=12)
                    _insert_at_index(12, account_id, account_code, account_name, [-a for a in amount])
                else:
                    # 'Cash out for unclassified activities'                (index=15)
                    _insert_at_index(15, account_id, account_code, account_name, [-a for a in amount])
        ...
        for account_id, account_code, account_name, balance in self._compute_liquidity_balance(beginning_period_options,
                                                                                               ...
                                                                                               payment_account_ids):
            if options['comparison'].get('periods'):
                balance = self.get_balance_from_periods(options, currency_table_query, account_id)
            else:
                balance = [balance]
        ...
        if options['comparison'].get('periods'):
            ...
            liquidity_result = {}
            for period in options['comparison'].get('periods'):
                res = self.get_liquidity_move_from_periods(options, period, currency_table_query, 'liquidity')
                for account_id, account_code, account_name, account_internal_type, amount in res:
                    if liquidity_result.get(account_id):
                        value = liquidity_result[account_id][4]
                        value[current_period] = amount
                        liquidity_result[account_id][4] = value
                    else:
                        value = [0.0] * period_number
                        value[current_period] = amount
                        liquidity_result[account_id] = [account_id, account_code, account_name, account_internal_type,
                                                        value]
                current_period += 1
            for account_id, account_code, account_name, account_internal_type, amount in liquidity_result.values():
        ...
        if options['comparison'].get('periods'):
            ...
            reconciled_result = {}
            for period in options['comparison'].get('periods'):
                res = self.get_liquidity_move_from_periods(options, period, currency_table_query, 'reconciled')
                for account_id, account_code, account_name, account_internal_type, amount in res:
                    if reconciled_result.get(account_id):
                        value = reconciled_result[account_id][4]
                        value[current_period] = amount
                        reconciled_result[account_id][4] = value
                    else:
                        value = [0.0] * period_number
                        value[current_period] = amount
                        reconciled_result[account_id] = [account_id, account_code, account_name, account_internal_type,
                                                         value]
                current_period += 1
            for account_id, account_code, account_name, account_internal_type, amount in reconciled_result.values():
                _dispatch_result(account_id, account_code, account_name, account_internal_type, amount)
        else:
        ...
        for i in range(0, (period_number + 1)):
            ...
            lines_to_compute[1]['columns'][i]['name'] = \
                lines_to_compute[2]['columns'][i]['name'] + \
                lines_to_compute[7]['columns'][i]['name'] + \
                lines_to_compute[10]['columns'][i]['name'] + \
                lines_to_compute[13]['columns'][i]['name']
            ...
            closing_ending_gap = lines_to_compute[16]['columns'][i]['name'] - lines_to_compute[0]['columns'][i]['name']
            computed_gap = sum(lines_to_compute[index]['columns'][i]['name'] for index in [2, 7, 10, 13])
            delta = closing_ending_gap - computed_gap
            if not self.env.company.currency_id.is_zero(delta):
                columns[i]['name'] = delta
        ...
        for line in lines_to_compute:
            ...
            # Header line.
            for i in range(0, (period_number + 1)):
                line['columns'][i]['name'] = self.format_value(line['columns'][i]['name'])
            lines.append(line)

            # Sub lines.
            for sub_line in sub_lines:
                for i in range(0, (period_number + 1)):
                    sub_line['columns'][i]['name'] = self.format_value(sub_line['columns'][i]['name'])

...
class AccountReport(models.AbstractModel):
    ...
    def _get_templates(self):
        return {
                'main_template': 'odex25_account_reports.main_template',
                'main_table_header_template': 'odex25_account_reports.main_table_header',
                'line_template': 'odex25_account_reports.line_template',
    ...
    def action_partner_reconcile(self, options, params):
        form = self.env.ref('odex25_account_accountant.action_manual_reconciliation', False)
        ctx = self.env.context.copy()
    ...
    def get_html_footnotes(self, footnotes):
        template = self._get_templates().get('footnotes_template', 'odex25_account_reports.footnotes_template')
    ...
    def open_report_export_wizard(self, options):
        ...
        view_id = self.env.ref('odex25_account_reports.view_report_export_wizard').id
    ...
    def get_pdf(self, options, minimal_layout=True):
        ...
        body = self.env['ir.ui.view']._render_template(
            "odex25_account_reports.print_template",

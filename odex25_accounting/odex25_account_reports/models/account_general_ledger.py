...
class AccountGeneralLedgerReport(models.AbstractModel):
    ...
    def _get_templates(self):
        ...
        templates['line_template'] = 'odex25_account_reports.line_template_general_ledger_report'
        templates['main_template'] = 'odex25_account_reports.main_template_with_filter_input_accounts'
        return templates
    ...
    def _get_query_sums(self, options_list, expanded_account=None):
        ...
        modules = self.env['ir.module.module'].sudo().search([('state', '=', 'installed'),
                                                              ('name', '=', 'exp_multi_unaffected_earnings')])
        if modules:
            queries.append('''
                SELECT
                    unaffected_earnings_account.earning_account_id          AS groupby,
                ...
                FROM %s
                LEFT JOIN %s ON currency_table.company_id = account_move_line.company_id
                LEFT JOIN account_account AS unaffected_earnings_account ON account_move_line.account_id = unaffected_earnings_account.id 
                WHERE %s
                GROUP BY unaffected_earnings_account.earning_account_id
            ''' % (i, tables, ct_query, where_clause))
        else:
            queries.append('''
                            SELECT

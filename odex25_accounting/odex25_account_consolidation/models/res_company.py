...
class Company(models.Model):
    ...
    odex25_account_consolidation_currency_is_different = fields.Boolean(
        compute='_compute_odex25_account_consolidation_currency_is_different')
    ...
    def _compute_odex25_account_consolidation_currency_is_different(self):
        for record in self:
            record.odex25_account_consolidation_currency_is_different = self._context.get('consolidation_currency_id') != record.currency_id.id
    ...
    def action_open_mapping(self):
        ...
        """
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("odex25_account_consolidation.account_mapping_action")
        action.update({
    ...
    def action_open_rate_ranges(self):
        ...
        """
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("odex25_account_consolidation.consolidation_rate_action")
        action.update({
            'context': {

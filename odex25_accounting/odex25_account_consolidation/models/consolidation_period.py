...
class ConsolidationPeriod(models.Model):
    ...
    def action_open_mapping(self):
        ...
        action.update({
            ...
            'views': [[self.env.ref('odex25_account_consolidation.account_mapping_tree').id, 'list']],
            'context': {
    ...
    def action_open_form(self):
        ...
        """
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("odex25_account_consolidation.consolidation_period_action")
        action.update({
            'views': [[False, 'form']],
            'res_id': self.id
        })
    ...
    def action_open_trial_balance_grid(self):
        ...
        return {
            ...
            'search_view_id': [self.env.ref('odex25_account_consolidation.trial_balance_grid_search').id, 'search']
        }

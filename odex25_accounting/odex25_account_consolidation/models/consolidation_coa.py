...
class ConsolidationChart(models.Model):
    ...
    children_ids = fields.Many2many('consolidation.chart', 'odex25_account_consolidation_inner_rel', 'children_ids',
                                    'parent_ids', string="Sub-consolidations")
    parents_ids = fields.Many2many('consolidation.chart', 'odex25_account_consolidation_inner_rel', 'parent_ids',
    ...
    def action_open_mapping(self):
        ...
        return {
            ...
            'view_mode': 'tree',
            'views': [[self.env.ref('odex25_account_consolidation.consolidation_account_tree_mapping').id, 'list']],
            'domain': [('chart_id', '=', self.id)],
            'context': {},
            'name': _('Account Mapping: %(chart)s', chart=self.name),
            'search_view_id': [self.env.ref('odex25_account_consolidation.consolidation_account_search_mapping').id, 'search']
        }
    ...
    def setting_consolidation_action(self):
        ...
        :return: the action to execute
        """
        action = self.env["ir.actions.actions"]._for_xml_id("odex25_account_consolidation.consolidation_chart_action_onboarding")
    ...
    def setting_consolidated_chart_of_accounts_action(self):
        ...
        :return: the action to execute
        """
        action = self.env["ir.actions.actions"]._for_xml_id("odex25_account_consolidation.consolidation_account_action")
        last_chart = self.search([], order="id desc", limit=1)
        action.update({
            'context': {'default_chart_id': last_chart.id, 'search_default_chart_id': last_chart.id},
            'views': [
                (self.env.ref('odex25_account_consolidation.consolidation_account_tree_onboarding').id, 'list'),
                (False, 'form')
            ]
        })
    ...
    def setting_create_period_action(self):
        ...
        :return: the action to execute
        """
        action = self.env["ir.actions.actions"]._for_xml_id("odex25_account_consolidation.consolidation_period_action_onboarding")
        last_chart = self.search([], order="id desc", limit=1)

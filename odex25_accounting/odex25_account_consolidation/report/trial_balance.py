...
class AccountConsolidationTrialBalanceReport(models.AbstractModel):
    ...
    def action_open_view_grid(self, options):
        ...
        return {
            ...
            'view_type': 'grid',
            'views': [
                [self.env.ref('odex25_account_consolidation.view_trial_balance_report_grid').id, 'grid'],
                [self.env.ref('odex25_account_consolidation.view_trial_balance_report_graph').id, 'graph'],
                [self.env.ref('odex25_account_consolidation.consolidation_journal_line_form').id, 'form']
            ],
            'context': {
    ...
    def action_open_audit(self, options, params=None):
        ...
        if len(journal_lines) == 0:
            return None
        action = self.env["ir.actions.actions"]._for_xml_id("odex25_account_consolidation.view_account_move_line_filter")
        action.update({
            'context': {
            ...
            'views': [(self.env.ref('odex25_account_consolidation.view_move_line_tree_grouped_general').id, 'list')]
        })
        return action
    ...
    def _get_journal_col(self, journal, options):
        ...
        if journal.currencies_are_different and journal.company_period_id:
            ...
            return {'name': journal.name, 'consolidation_rate': journal.rate_consolidation,
                    ...
                    'template': 'odex25_account_consolidation.header_cell_template'}
    ...
    def _get_templates(self):
        return {
            'main_template': 'odex25_account_consolidation.main_template_conso_report',
            'main_table_header_template': 'odex25_account_reports.main_table_header',
            'line_template': 'odex25_account_consolidation.line_template',
            'footnotes_template': 'odex25_account_reports.footnotes_template',
            'search_template': 'odex25_account_reports.search_template',
        }

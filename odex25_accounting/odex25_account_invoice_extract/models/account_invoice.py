...
class AccountMove(models.Model):
    ...
    def _contact_iap_extract(self, local_endpoint, params):
        ...
        endpoint = self.env['ir.config_parameter'].sudo().get_param('odex25_account_invoice_extract_endpoint', EXTRACT_ENDPOINT)
        ...
        if attachments and attachments.exists() and self.move_type in ['in_invoice', 'in_refund'] and self.extract_state in ['no_extract_requested', 'not_enough_credit', 'error_status', 'module_not_up_to_date']:
            ...
            try:
                ...
                if result['status_code'] == SUCCESS:
                    if self.env['ir.config_parameter'].sudo().get_param("odex25_account_invoice_extract.already_notified", True):
                        self.env['ir.config_parameter'].sudo().set_param("odex25_account_invoice_extract.already_notified", False)
    ...
    def send_no_credit_notification(self):
        ...
        already_notified = self.env['ir.config_parameter'].sudo().get_param("odex25_account_invoice_extract.already_notified", True)
        if already_notified:
            return
        try:
            mail_template = self.env.ref('odex25_account_invoice_extract.odex25_account_invoice_extract_no_credit')
        except ValueError:
        ...
        if iap_account:
            ...
            if res:
                ...
                self.env['ir.config_parameter'].sudo().set_param("odex25_account_invoice_extract.already_notified", True)
    ...
    def _create_supplier_from_vat(self, vat_number_ocr):
        ...
        if response and response.get('company_data'):
            ...
            # odex25_account_invoice_extract

...
class AccountBudgetPost(models.Model):
    ...
    #                 raise ValidationError(_('This Account %s is arleady exist in another budget.', account.name))
...
class CrossoveredBudget(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company', required=True, tracking=True,
                                 default=lambda self: self.env.company)
...
class CrossoveredBudgetLines(models.Model):
    ...
    date_from = fields.Date('Start Date', required=True ,related='crossovered_budget_id.date_from',)
    date_to = fields.Date('End Date', required=True , related='crossovered_budget_id.date_to',)
    ...
    def action_open_budget_entries(self):
        if self.analytic_account_id:
            ...
            action['domain'] = ['|', ('account_id', '=', self.analytic_account_id.id),
                                ('account_id.parent_id', 'child_of', self.analytic_account_id.id),
                                ('date', '>=', self.date_from),
                                ('date', '<=', self.date_to)
                                ]
            ...
            action['domain'] = ['|', ('account_id', 'in', self.general_budget_id.account_ids.ids),
                                ('account_id.parent_id', 'child_of', self.general_budget_id.account_ids.ids),
                                ('date', '>=', self.date_from),
                                ('date', '<=', self.date_to)
                                ]
        return action

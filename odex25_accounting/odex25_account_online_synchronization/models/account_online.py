...
from odoo.addons.odex25_account_online_synchronization.models.odoofin_auth import OdooFinAuth
...
class AccountOnlineLink(models.Model):
    ...
    def _compute_next_synchronization(self):
        for rec in self:
            rec.next_refresh = self.env.ref('odex25_account_online_synchronization.online_sync_cron').sudo().nextcall
    ...
    def _fetch_odoo_fin(self, url, data=None, ignore_status=False):
        ...
        timeout = int(self.env['ir.config_parameter'].sudo().get_param('odex25_account_online_synchronization.request_timeout')) or 60
        proxy_mode = self.env['ir.config_parameter'].sudo().get_param('odex25_account_online_synchronization.proxy_mode') or 'production'

...
QUnit.module('FollowupViews', {
    ...
    QUnit.test('followup form view do it later', async function (assert) {
        ...
        var $buttonDoItLater = $buttons.find('button.o_odex25_account_followup_do_it_later_button');
        var $buttonDone = $buttons.find('button.o_odex25_account_followup_done_button');
    ...
    QUnit.test('followup form print letter', async function (assert) {
        ...
        var $buttonPrintLetter = followupFormView.$buttons.find('button.o_odex25_account_followup_print_letter_button');
        assert.strictEqual(followupFormView.$buttons.find('button.o_odex25_account_followup_done_button').css('display'), 'none',
            "Done button should be invisible");
        ...
        assert.strictEqual(followupFormView.$buttons.find('button.o_odex25_account_followup_done_button').css('display'), 'inline-block',
            "Done button should be visible after print");

        await testUtils.dom.click(followupFormView.$buttons.find('button.o_odex25_account_followup_done_button'));
    ...
    QUnit.test('followup form view multiple records', async function (assert) {
        ...
        assert.strictEqual(cpHelpers.getPagerValue(followupFormView), "1", 'pager value should be 1');
        assert.strictEqual(cpHelpers.getPagerSize(followupFormView), "4", 'pager limit should be 4');

        await cpHelpers.pagerNext(followupFormView);

        assert.strictEqual(cpHelpers.getPagerValue(followupFormView), "2", 'pager value should be 2');

        var $buttonDoItLater = followupFormView.$buttons.find('button.o_odex25_account_followup_do_it_later_button');
        await testUtils.dom.click($buttonDoItLater);

        assert.strictEqual(cpHelpers.getPagerValue(followupFormView), "2", 'pager value should be 2');
        assert.strictEqual(cpHelpers.getPagerSize(followupFormView), "3", 'pager limit should be 3');

...
class AccountFollowupReport(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        ...
        headers = [{},
                   ...
                   {'name': _('Analytic Account'), 'style': 'text-align:right; white-space:nowrap;'},
                   {'name': _('Analytic Group'), 'style': 'text-align:right; white-space:nowrap;'},
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for currency, aml_recs in res.items():
            ...
            for aml in aml_recs:
                ...
                analytic_account = aml.move_id.analytic_account_id.display_name
                if self.env.context.get('print_mode'):
                    analytic_account = {'name': analytic_account, 'style': 'text-align:right; white-space:normal;'}
                analytic_account_group = aml.move_id.analytic_account_id.group_id.name
                if self.env.context.get('print_mode'):
                    analytic_account_group = {'name': analytic_account_group,
    ...
    def _get_templates(self):
        ...
        templates['main_template'] = 'odex25_account_followup.template_followup_report'
        templates['line_template'] = 'odex25_account_followup.line_template_followup_report'
        return templates
    ...
    def print_followups(self, records):
        ...
        action = self.env.ref('odex25_account_followup.action_report_followup').report_action(res_ids)
    ...
    def get_followup_line(self, options):
        ...
        if options.get('followup_level'):
            followup_line = self.env['odex25_account_followup.followup.line'].browse(options['followup_level'][0])
    ...
    def do_manual_action(self, options):
        ...
        partner = self.env['res.partner'].browse(options.get('partner_id'))
        if options.get('followup_level'):
            followup_line = self.env['odex25_account_followup.followup.line'].browse(options.get('followup_level'))
            if followup_line:

...
class ResPartner(models.Model):
    ...
    followup_level = fields.Many2one('odex25_account_followup.followup.line', compute="_compute_for_followup", string="Follow-up Level")
    ...
    def _compute_for_followup(self):
        ...
        """
        first_followup_level = self.env['odex25_account_followup.followup.line'].search([('company_id', '=', self.env.company.id)], order="delay asc", limit=1)
    ...
    def open_action_followup(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'views': [[self.env.ref('odex25_account_followup.customer_statements_form_view').id, 'form']],
    ...
    def _compute_followup_lines(self):
         ...
         * keys being the different possible levels of followup for account.move.line's (None or IDs of odex25_account_followup.followup.line)
        ...
        """
        followup_line_ids = self.env['odex25_account_followup.followup.line'].search([('company_id', '=', self.env.company.id)], order="delay asc")
    ...
    def _query_followup_level(self, all_partners=False):
        ...
        sql = """
            ...
            LEFT OUTER JOIN odex25_account_followup_followup_line current_followup_level ON current_followup_level.id = (
                SELECT COALESCE(next_ful.id, ful.id) FROM unreconciled_aml aml
                LEFT OUTER JOIN odex25_account_followup_followup_line ful ON ful.id = aml.followup_line_id
                LEFT OUTER JOIN odex25_account_followup_followup_line next_ful ON next_ful.id = (
                    SELECT next_ful.id FROM odex25_account_followup_followup_line next_ful
        ...
        self.env['odex25_account_followup.followup.line'].flush()

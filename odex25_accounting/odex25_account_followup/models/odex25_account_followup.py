...
class FollowupLine(models.Model):
    _name = 'odex25_account_followup.followup.line'
    ...
    sms_description = fields.Char('SMS Text Message', translate=True, default=lambda s: _(
        "Dear %(partner_name)s, it seems that some of your payments stay unpaid"))
...
Best Regards,
    ...
    send_sms = fields.Boolean('Send an SMS Message', help="When processing, it will send an sms text message",
                              default=False)
        ...
        for line in self:
            if line.description:
                try:
                    ...
                    raise Warning(
                        _('Your description is invalid, use the right legend or %% if you want to use the percent character.'))
        ...
        for line in self:
            if line.sms_description:
                try:
                    ...
                    raise Warning(
                        _('Your sms description is invalid, use the right legend or %% if you want to use the percent character.'))
    ...
    def _get_next_date(self):
        self.ensure_one()
        next_followup = self.env['odex25_account_followup.followup.line'].search([('delay', '>', self.delay),
                                                                                  ('company_id', '=',
                                                                                   self.env.company.id)],
    ...
    def _compute_analytic_account_id(self):
        for rec in self:
            if rec.invoice_line_ids:
                rec.analytic_account_id = rec.invoice_line_ids[0].analytic_account_id.id
            else:
...
class AccountMoveLine(models.Model):
    ...
    followup_line_id = fields.Many2one('odex25_account_followup.followup.line', 'Follow-up Level', copy=False)

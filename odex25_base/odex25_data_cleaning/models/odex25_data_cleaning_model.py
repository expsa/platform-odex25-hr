...
class DataCleaningModel(models.Model):
    _name = 'odex25_data_cleaning.model'
    ...
    rule_ids = fields.One2many('odex25_data_cleaning.rule', 'cleaning_model_id', string='Rules')
    ...
    def _compute_records_to_clean(self):
        count_data = self.env['odex25_data_cleaning.record'].read_group(
    ...
    def _clean_records_format_phone(self, actions, field):
        ...
        PhoneFormat = self.env['odex25_data_cleaning.record'].phone_format

        self._cr.execute("""
            SELECT res_id, odex25_data_cleaning_rule_id
            FROM odex25_data_cleaning_record
            JOIN odex25_data_cleaning_record_odex25_data_cleaning_rule_rel
            ON odex25_data_cleaning_record_odex25_data_cleaning_rule_rel.odex25_data_cleaning_record_id = odex25_data_cleaning_record.id""")
        ...
        for record in records:
            record_country = self.env['odex25_data_cleaning.record']._get_country_id(record)
    ...
    def _clean_records(self):
        ...
        for cleaning_model in self:
            ...
            for field in actions:
                ...
                if cleaner:
                    ...
                    """).format(
                        ...
                        cleaning_record_table=sql.Identifier(self.env['odex25_data_cleaning.record']._table),
            ...
            if cleaning_model.cleaning_mode == 'automatic':
                self.env['odex25_data_cleaning.record'].create(records_to_create).action_validate()
            else:
                records_to_clean = records_to_clean + records_to_create
        self.env['odex25_data_cleaning.record'].create(records_to_clean)
    ...
    def _send_notification(self, delta):
        ...
        records_count = self.env['odex25_data_cleaning.record'].search_count([
            ('cleaning_model_id', '=', self.id),
        ...
        if records_count:
            ...
            template = self.env.ref('odex25_data_cleaning.notification')
            menu_id = self.env.ref('odex25_data_cleaning.menu_odex25_data_cleaning_root').id
            kwargs = {
    ...
    def write(self, vals):
        if 'active' in vals and not vals['active']:
            self.env['odex25_data_cleaning.record'].search([('cleaning_model_id', 'in', self.ids)]).unlink()
    ...
    def open_records(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("odex25_data_cleaning.action_odex25_data_cleaning_record")

...
class DataCleaningRecord(models.Model):
    _name = 'odex25_data_cleaning.record'
    _description = 'Cleaning Record'

    active = fields.Boolean('Active', default=True)
    name = fields.Char('Record Name', compute='_compute_values', compute_sudo=True)
    rule_ids = fields.Many2many('odex25_data_cleaning.rule', string='Rule', required=True, ondelete='cascade')
    field_id = fields.Many2one('ir.model.fields', string='Field')
    cleaning_model_id = fields.Many2one('odex25_data_cleaning.model', string='Cleaning Model')
    ...
    def action_validate(self):
        records_done = self.env['odex25_data_cleaning.record']

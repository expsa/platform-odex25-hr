...
class LeaveController(http.Controller):
    ...
    def get_leaves(self, approvel=None, page=None, **kw):
        ...
        try:
            ...
            """Search for alternative employees,this is used in leaves that need alternative employee """
            alternative_employees = http.request.env['hr.employee'].sudo().search_read(
                [('state', '=', 'open'), ('id', '!=', employee.id)], ['name'])
            """Search for  Employees reporting to the current employee or  coached by the current employee in the same department """
            ...
            if status:
                for s in status:
                    ...
                    value = {'id': s.id, 'name': s.name, 'ticket': s.issuing_ticket,

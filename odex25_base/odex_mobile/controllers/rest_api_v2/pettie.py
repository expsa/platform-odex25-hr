...
class PettiesController(http.Controller):
    # Document
    @http.route(['/rest_api/v2/documents'], type='http', auth='none', csrf=False, methods=['GET'])
    ...
    # Petty
    @http.route(['/rest_api/v2/petties'], type='http', auth='none', csrf=False, methods=['GET'])
    ...
    @http.route(['/rest_api/v2/pettie/<string:id>'], type='http', auth='none', csrf=False, methods=['GET'])
    def get_petties(self,id,**kw):
        http_method, body, headers, token = http_helper.parse_request()
        result = validator.verify_token(token)
        if not result['status']:
            return http_helper.errcode(code=result['code'], message=result['message'])
        ...
        try:
            ...
            return http_helper.response(message="Data Found", data={'petties': value,'types':types})
        except Exception as e:
            _logger.error(str(e))
            message = validator.get_server_error(e, user)
            return http_helper.errcode(code=403, message=message)
    ...
    #petty create
    @http.route(['/rest_api/v2/petties'], type='http', auth='none', csrf=False, methods=['POST'])
    ...
    #petty edit
    @http.route(['/rest_api/v2/petties/<string:id>'], type='http', auth='none', csrf=False, methods=['PUT'])
    ...
    @http.route(['/rest_api/v2/petties/<string:pettyId>'], type='http', auth='none', csrf=False,methods=['DELETE'])

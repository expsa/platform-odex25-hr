...
class generic_tax_report_inherit(models.AbstractModel):
    ...
    def _compute_from_amls_taxes(self, options, dict_to_fill, period_number):
        ...
        for res in self.env.cr.fetchall():
            if res[0] in dict_to_fill:
                if 'periods' not in dict_to_fill[res[0]]:
                    dict_to_fill[res[0]]['periods'] = {}
                if period_number not in dict_to_fill[res[0]]['periods']:
                    dict_to_fill[res[0]]['periods'][period_number] = {'net': 0, 'tax': 0}
                dict_to_fill[res[0]]['periods'][period_number]['net'] += res[1]
            # dict_to_fill[res[0]]['periods'][period_number]['net'] += res[1]
            # dict_to_fill[res[0]]['show'] = True

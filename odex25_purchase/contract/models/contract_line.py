...
class ContractLine(models.Model):
    ...
    def _compute_first_recurring_next_date(
        ...
        if recurring_invoicing_type == 'pre-paid':
            return date_start
        return fields.Date.from_string(date_start) + self.get_relative_delta(
    ...
    def _prepare_invoice_line(self, invoice_id=False):
        ...
        invoice_line_vals = {
            ...
            'tax_ids' : self.tax_id and [(6,0,[self.tax_id.id])]
    ...
    def _get_period_to_invoice(
        ...
        first_date_invoiced = (
            fields.Date.from_string(last_date_invoiced) + relativedelta(days=1)
            if last_date_invoiced
            else fields.Date.from_string(self.date_start)
        )
        if self.recurring_rule_type == 'monthlylastday':
            ...
            if self.recurring_invoicing_type == 'pre-paid':
                last_date_invoiced = (
                    fields.Date.from_string(recurring_next_date)
                    + self.get_relative_delta(
                ...
                last_date_invoiced = fields.Date.from_string(recurring_next_date) - relativedelta(
                    days=1
                )
        if stop_at_date_end:
            if self.date_end and fields.Date.from_string(self.date_end) < last_date_invoiced:
                last_date_invoiced = fields.Date.from_string(self.date_end)
        return first_date_invoiced, last_date_invoiced, recurring_next_date
    ...
    def _update_recurring_next_date(self, recurring_next_date=None):
        for rec in self:
            if not rec.recurring_next_date:
                rec.recurring_next_date = recurring_next_date
            
            old_date = fields.Date.from_string(rec.recurring_next_date)
            diff_date = self.get_relative_delta(
                rec.recurring_rule_type, rec.recurring_interval
            )

            new_date = old_date + diff_date
            ...
            if date_end and last_date_invoiced >= rec.date_end:
    ...
    def action_plan_successor(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'name': _('Plan contract line successor'),
            'res_model': 'contract.line.wizard',
            'view_mode': 'form',
            'views': [(view_id, 'form')],
            'target': 'new',
    ...
    def action_stop(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'name': _('Resiliate contract line'),
            'res_model': 'contract.line.wizard',
            'view_mode': 'form',
            'views': [(view_id, 'form')],
            'target': 'new',
    ...
    def _get_renewal_dates(self):
        self.ensure_one()
        date_start = fields.Date.from_string(self.date_end) + relativedelta(days=1)

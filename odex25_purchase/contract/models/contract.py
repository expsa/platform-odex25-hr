...
class ContractContract(models.Model):
    ...
    analytic_account_id = fields.Many2one(
        string="Cost Center",
        comodel_name='account.analytic.account',
        compute="get_line_analytic_account"
    )
    ...
    def _get_report_base_filename(self):
        return "{} - {}".format(self.name, self.partner_id.name)
    ...
    def _get_related_payment(self):
        self.ensure_one()
        payments = self.env['account.payment'].search([('contract_id', '=', self.id)])
        return payments
    ...
    def _compute_payment_count(self):
        for rec in self:
            rec.payment_count = len(rec._get_related_payment())
    ...
    @api.model
    def get_recurring_next_date(self):
        ...
        if recurring_next_date:
            self.recurring_next_date = min(recurring_next_date)

        else:
            self.recurring_next_date = self.date_start or self.date

    def _compute_recurring_next_date(self):
        for contract in self:
            contract.get_recurring_next_date()
    ...
    def get_line_analytic_account(self):
        for rec in self:
            rec.get_default_analytic_account()

    @api.onchange("contract_line_ids")
    def _onchange_contract_line_ids(self):
        self.get_default_analytic_account()
        self.get_recurring_next_date()
    ...
    def _prepare_invoice(self, date_invoice, name_seq, journal=None):
        self.ensure_one()
        if not journal:
            journal = (
                self.journal_id
    ...
    def _finalize_invoice_values(self, invoice_values):
        ...
        for invoice_line in new_invoice.invoice_line_ids:
            ...
            account_analytic_id = invoice_line.analytic_account_id
            price_unit = invoice_line.price_unit
            invoice_line.move_id = new_invoice.id
    ...
    def _finalize_and_create_invoices(self, invoices_values):
        ...
        invoices = self.env['account.move'].with_context(
            check_move_validity=False).create(final_invoices_values[0])
        for invoice in invoices:
    ...
    def _get_data_invoice_line(self, line):
    ...
    def _prepare_recurring_invoices_values(self, date_ref=False):
        ...
        the lines to invoice of the selfs in self.
        ...
        invoice_line_values = []
        contract_lines = self.contract_line_ids

        sequence_code = 'contract.contract_inv.seq'
        name_seq = self.env['ir.sequence'].next_by_code(sequence_code)

        journal = self.journal_id
        invoice_values = self._prepare_invoice(date_ref, name_seq, journal)
        # invoice_line_values = [(0, 0, self._get_data_invoice_line(line)) for line in contract_lines]
        ...
        print("tax:", tax, tax.ids)

        invoice_line_values = [(0, 0, {
            'name': self.name,
        ...
        invoice_values['invoice_line_ids'] = invoice_line_values
    ...
    def cron_recurring_create_invoice(self):
        ...
        for contract in contracts_to_invoice:
            date_ref = fields.Date.context_today(contract)
...
class AccountPayment(models.Model):
    ...
    def create(self, vals):
        ...
        # active_ids
        move_id = self.env['account.move'].search([('id', '=', context.get('active_id', False))])
        rec = super(AccountPayment, self).create(vals)

        if move_id:
            rec.update({
                'contract_id': move_id.contract_id.id,
            })
            # move_id.write({
            #     'payment_id': rec.id,
            # })

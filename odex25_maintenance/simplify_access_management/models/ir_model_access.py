...
{resolution_info}""".format(
        ...
        try:
            ...
            if self.env.user.id and read_value:
                a = "select access_management_id from access_management_comapnay_rel where company_id = " + str(request.httprequest.cookies.get('cids') and request.httprequest.cookies.get('cids').split(',')[0] or request.env.company.id or 0)
                self._cr.execute(a)
                a = self._cr.fetchall()
                if a:    

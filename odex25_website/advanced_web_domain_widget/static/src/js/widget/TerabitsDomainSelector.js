odoo.define("advanced_web_domain_widget.TerabitsDomainSelector", function (require) {
     ...
     * and/or leaf parts of a domain (@see TerabitsDomainLeaf).
     */
        ...
        _renderChildrenTo: function ($to) {
            ...
            return Promise.all(_.map(this.children, (function (child) {
                ...
                _.each(this.children, function (child) {
                    ...
                    // Forced to do it this way so that the
    ...
    var TerabitsDomainLeaf = TerabitsDomainNode.extend({
        ...
        start: function () {
            ...
            var selectedField = this.fieldSelector.getSelectedField() || {}
            if ((_.contains(['many2one', 'many2many', 'one2many'], selectedField.type) || selectedField.name == "id") && (_.contains(['in', 'not in'], this.operator))) {
            ...
            if (!this.readonly && this.valueWidget && this.operator != "date_filter") { // In edit mode, place the value widget if any
        ...
        _changeFieldChain: function (chain, silent) {
            ...
            this.fieldSelector.setChain(chain).then((function () {
                ...
                var selectedField = this.fieldSelector.getSelectedField() || {};
                var operators = this._getOperatorsFromType(selectedField.type, selectedField.name);
        ...
        _changeOperator: function (operator, silent) {
            ...
            if (_.contains(["set", "not set"], this.operator)) {
                ...
                this.value = false;
            } else if(this.operator == "date_filter") {
                this.value = "today";
        ...
        _changeValue: function (value, silent) {
            ...
            } else if ((_.contains(["date", "datetime"], selectedField.type)) && this.operator != "date_filter") {
            ...
            } else if ((_.contains(['many2many', 'one2many', 'many2one'], selectedField.type) || selectedField.name == "id") && (_.contains(['in', 'not in'], this.operator))) {
         ...
         */
        _getOperatorsFromType: function (type, name) {
            var operators = {};
            if(name == "id") {
            ...
            switch (type) {
                ...
                case "date":
                case "datetime":
                    operators = _.pick(operator_mapping, "=", "!=", ">", "<", ">=", "<=", "set", "not set", "date_filter");
                    break;
     ...
     * TerabitsDomainLeaf if it only contains one part. Returns null otherwise.
     *
     * @param {Object} parent
     * @param {string} model - the model name
     * @param {Array|string} domain - the prefix representation of the domain
     * @param {Object} options - @see TerabitsDomainNode.init.options
     * @returns {TerabitsDomainTree|TerabitsDomainLeaf|null}
     */

odoo.define('odex25_sign.PDFIframe', function (require) {
    'use strict';
    ...
    var PDFIframe = Widget.extend(Object.assign({}, PinchItemMixin, {
        ...
        doPDFPostLoad: function() {
            ...
            var assets_def = this._rpc({
                route: '/odex25_sign/render_assets_pdf_iframe',
                params: {
                    args: [{
        ...
        createSignItem: function (type, required, responsible, posX, posY, width, height, value, option_ids, name, tooltip) {
            ...
            var $signatureItem = $(core.qweb.render('odex25_sign.sign_item', {
...
odoo.define('odex25_sign.Document', function (require) {
    'use strict';

    var ajax = require('web.ajax');
    var core = require('web.core');
    var Dialog = require('web.Dialog');
    var PDFIframe = require('odex25_sign.PDFIframe');
    var Widget = require('web.Widget');
...
odoo.define('odex25_sign.utils', function (require) {
    'use strict';
...
// Signing part
odoo.define('odex25_sign.document_signing', function (require) {
    'use strict';
    ...
    var core = require('web.core');
    var Dialog = require('web.Dialog');
    var Document = require('odex25_sign.Document');
    ...
    var SignNameAndSignature = NameAndSignature.extend({
        ...
        willStart: function () {
            ...
            return Promise.all([
                this._super.apply(this, arguments),
                self._rpc({
                    route: '/odex25_sign/get_signature/' + self.requestID + '/' + self.accessToken,
                    params: {
    ...
    var SignatureDialog = Dialog.extend({
        template: 'odex25_sign.signature_dialog',
    ...
    var PublicSignerDialog = Dialog.extend({
        template: "odex25_sign.public_signer_dialog",
        ...
        init: function(parent, requestID, requestToken, RedirectURL, options) {
            ...
            if(!options.buttons) {
                ...
                options.buttons.push({text: _t("Validate & Send"), classes: "btn-primary", click: function(e) {
                    ...
                    ajax.jsonRpc("/odex25_sign/send_public/" + this.requestID + '/' + this.requestToken, 'call', {
                        name: name,
                        mail: mail,
                    }).then(function() {
                        self.close();
    ...
    var SMSSignerDialog = Dialog.extend({
        template: "odex25_sign.public_sms_signer",

        events: {
            'click button.o_sign_resend_sms': function(e) {
                var $btn = self.$('.o_sign_resend_sms');
                $btn.attr('disabled', true);
                var route = '/odex25_sign/send-sms/' + this.requestID + '/' + this.requestToken + '/' + this.$('#o_sign_phone_number_input').val();
        ...
        _onValidateSMS: function () {
            ...
            var route = '/odex25_sign/odex25_sign/' + this.requestID + '/' + this.requestToken + '/' + input.val();
            var params = {
    ...
    var EncryptedDialog = Dialog.extend({
        template: "odex25_sign.public_password",
        ...
        _onValidatePassword: function () {
            ...
            var route = '/odex25_sign/password/' + this.requestID ;
            var params = {
                password: input.val()
            };
            var self = this;
    ...
    var ThankYouDialog = Dialog.extend({
        ...
        init: function(parent, RedirectURL, RedirectURLText, requestID, options) {
            ...
            this._rpc({
                route: '/odex25_sign/encrypted/' + requestID
        ...
        start: async function () {
            var self = this;
            var result = false;
            var canReadRequestItem = await session.user_has_group('odex25_sign.group_sign_user');
            ...
            this.setElement($(core.qweb.render('odex25_sign.thank_you_dialog', {widget: this})));
    ...
    var NextDirectSignDialog = Dialog.extend({
        template: "odex25_sign.next_direct_sign_dialog",
        events: {
            'click .o_go_to_document': 'on_closed',
    ...
    var SignableDocument = Document.extend({
        ...
        signItemDocument: function(e) {
            ...
            function _sign() {
                ...
                var route = '/odex25_sign/odex25_sign/' + this.requestID + '/' + this.accessToken;
                var params = {
        ...
        signDocument: function (e) {
            ...
            signDialog.open().onConfirm(function () {
                ...
                function _sign() {
                    ajax.jsonRpc('/odex25_sign/odex25_sign/' + self.requestID + '/' + self.accessToken, 'call', {
                        signature: signature,
                ...
                return documentPage.attachTo($('body')).then(function() {
                    ...
                    if(askLocation && navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(function(position) {
                            ...
                            ajax.jsonRpc('/odex25_sign/save_location/' + documentPage.requestID + '/' + documentPage.accessToken, 'call', coords);
                        });
                    }
                });
            });

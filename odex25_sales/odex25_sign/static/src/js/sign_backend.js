odoo.define('odex25_sign.views_custo', function(require) {
    'use strict';
    ...
    function _make_custo(selector_button) {
        return {
            ...
            _sign_upload_file_button: function () {
                ...
                session.user_has_group('odex25_sign.group_sign_user').then(function (has_group) {
                    if (has_group) {
...
odoo.define('sign.template', function(require) {
    ...
    var sign_utils = require('odex25_sign.utils');
    ...
    var SignItemCustomPopover = Widget.extend({
        template: 'odex25_sign.sign_item_custom_popover',
        events: {
    ...
    var InitialAllPagesDialog = Dialog.extend({
        template: 'odex25_sign.initial_all_pages_dialog',
            ...
            this.fullyLoaded.then(function() {
                if(self.editMode) {
                    if(self.$iframe.prop('disabled')) {
                        ...
                        var $fieldTypeButtons = $(core.qweb.render('odex25_sign.type_buttons', {sign_item_types: typesArr}));
    ...
    var Template = AbstractAction.extend(StandaloneFieldManagerMixin, {
        ...
        events: {
            ...
            'click .o_sign_template_send' : function (e) {
                this.do_action('odex25_sign.action_sign_send_request', {
                    additional_context: {
            ...
            'click .o_sign_template_share' : function (e) {
                this.do_action('odex25_sign.action_sign_template_share', {
                    additional_context: {
                        active_id: this.templateID,
                    },
                });
            ...
            'click .o_sign_template_save' : function (e) {
                return this.do_action('odex25_sign.sign_template_action', {
...
odoo.define('odex25_sign.DocumentBackend', function (require) {
    'use strict';
    var AbstractAction = require('web.AbstractAction');
    var core = require('web.core');
    ...
    var DocumentBackend = AbstractAction.extend({
        ...
        go_back_to_kanban: function () {
            return this.do_action("odex25_sign.sign_request_action", {
                clear_breadcrumbs: true,
            });
        },
        ...
        start: function () {
            ...
            var def = this._rpc({
                route: '/odex25_sign/get_document/' + this.documentID + '/' + this.token,
...
odoo.define('odex25_sign.document_edition', function(require) {
    'use strict';

    var core = require('web.core');
    var session = require('web.session');
    var DocumentBackend = require('odex25_sign.DocumentBackend');
    ...
    core.action_registry.add('odex25_sign.Document', EditableDocumentBackend);
});

odoo.define('odex25_sign.document_signing_backend', function(require) {
    'use strict';
    ...
    var NoPubThankYouDialog = document_signing.ThankYouDialog.extend({
        template: "odex25_sign.no_pub_thank_you_dialog",
    ...
    core.action_registry.add('odex25_sign.SignableDocument', SignableDocumentBackend);
});

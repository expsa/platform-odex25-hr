...
odoo.define("pos_orders_history.models", function(require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        on_orders_history_updates: function(message) {
            ...
            .then(function() {
                return self.fetch_order_history_lines_by_order_ids(order_ids).then(
                    function(lines) {
    ...
    models.load_models({
        ...
        loaded: function(self, lines) {
            lines.forEach(line => {
                line.image = window.location.origin + "/web/image?model=product.product&field=image_128&id=" + line.product_id[0];
            });
            self.update_orders_history_lines(lines);
        },
    });

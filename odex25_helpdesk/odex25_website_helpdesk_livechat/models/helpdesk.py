    ...
    def _execute_command_helpdesk(self, **kwargs):
        ...
        if key[0].lower() == '/helpdesk':
            if len(key) == 1:
                ...
                link_ticket = '<a href="#" data-oe-id='+str(helpdesk_ticket.id)+' data-oe-model="odex25_helpdesk.ticket">'+helpdesk_ticket.name+'</a>'
                ...
                if not tickets:
                    for Keyword in Keywords:
                        ...
                        if len(tickets) > 10:
                            break
                if tickets:
                    link_tickets = ['<br/><a href="#" data-oe-id='+str(ticket.id)+' data-oe-model="odex25_helpdesk.ticket">#'+ticket.name+'</a>' for ticket in tickets]

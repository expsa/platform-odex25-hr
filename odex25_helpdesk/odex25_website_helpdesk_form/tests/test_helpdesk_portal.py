...
class HelpDeskPortal(HttpCase):
    ...
    def setUp(self):
        ...
        self.team_with_sla = self.env['odex25_helpdesk.team'].create({
            'name': 'Team with SLAs',
            'use_sla': True,
            'use_website_helpdesk_form': True,
            'is_published': True,
        })
        self.stage_new = self.env['odex25_helpdesk.stage'].create({
            'name': 'New',
            'sequence': 10,
            'team_ids': [(4, self.team_with_sla.id, 0)],
            'is_close': False,
        })
        self.sla = self.env['odex25_helpdesk.sla'].create({
    ...
    def test_portal_ticket_submission(self):
        ...
        ticket = self.env['odex25_helpdesk.ticket'].browse(response.json().get('id'))

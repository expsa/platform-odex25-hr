...
class HelpdeskTeam(models.Model):
    ...
    def _ensure_submit_form_view(self):
        for team in self:
            if not team.website_form_view_id:
                default_form = etree.fromstring(self.env.ref('odex25_website_helpdesk_form.ticket_submit_form').arch)
                xmlid = 'odex25_website_helpdesk_form.team_form_' + str(team.id)
                ...
                self.env['ir.model.data'].create({
                    'module': 'odex25_website_helpdesk_form',
                    'name': xmlid.split('.')[1],
                    'model': 'ir.ui.view',
    ...
    def _compute_form_url(self):
        ...
        for team in self:
            team.feature_form_url = (team.use_website_helpdesk_form and team.name and team.id) and (base_url + '/odex25_helpdesk/' + slug(team)) or False

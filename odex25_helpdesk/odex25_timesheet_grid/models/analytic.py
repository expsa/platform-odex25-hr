...
from odoo.addons.odex25_web_grid.models.models import END_OF, STEP_BY, START_OF
...
class AnalyticLine(models.Model):
    ...
    def _action_open_to_validate_timesheet_view(self, type_view='week'):
        ...
        action.update({
            ...
            "display_name": name,
            "views": [
                [self.env.ref('odex25_timesheet_grid.timesheet_view_grid_by_employee_validation').id, 'grid'],
                [self.env.ref('hr_timesheet.timesheet_view_tree_user').id, 'tree'],
                [self.env.ref('odex25_timesheet_grid.timesheet_view_form').id, 'form']
            ],
            "view_mode": 'grid,tree',
            "domain": [('is_timesheet', '=', True)],
            "search_view_id": [self.env.ref('odex25_timesheet_grid.timesheet_view_search').id, 'search'],
            "context": context,
    ...
    def action_merge_timesheets(self):
        ...
        return {
            ...
            'views': [(self.env.ref('odex25_timesheet_grid.timesheet_merge_wizard_view_form').id, 'form')],

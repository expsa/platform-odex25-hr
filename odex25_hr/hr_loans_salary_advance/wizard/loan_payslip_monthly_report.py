...
class PayslipMonthlyReport(models.TransientModel):
    ...
    def get_loan_lines(self):
        dom = []
        if self.salary_ids:
            dom += [('salary_scale', 'in', self.salary_ids.ids)]
        if self.level_ids:
            dom += [('salary_level', 'in', self.level_ids.ids)]
        if self.group_ids:
            dom += [('salary_group', 'in', self.group_ids.ids)]
        if self.degree_ids:
            dom += [('salary_degree', 'in', self.degree_ids.ids)]
    ...
    def check_data(self):
        if self.no_loan and self.no_rule:
            ...
            if not loan_lines:
                raise ValidationError(_('Sorry No Loans Data To Be Printed'))
            ...
            if self.detailed and self.listed:
                ...
                emp_ids = self.employee_ids and self.employee_ids.ids or \
                          list(set(r.deduction_line.employee_id.id for r in
                                   self.env['loan.installment.line'].browse().search([

...
class PayslipMonthlyReport(models.AbstractModel):
    ...
    def get_loan_values(self, data=None):
        ...
        if data['delist'] == 'ff':
            ...
            if not data['loan_line_ids'] or not data['loan_ids']: raise ValidationError(
                _('Sorry No Loan Data To Be Printed'))
...
class PayslipMonthlyReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        if not data['form']['no_loan'] and not data['form']['no_rule']:
            rtitle, rexception, docs_rule = self.env['report.exp_payroll_custom.payslip_monthly_report'].with_context(
                track_emp=True).get_rule_values(data)
            ltitle, exception, docs = self.env['report.exp_payroll_custom.payslip_monthly_report'].with_context(
                track_emp=True).get_loan_values(data)
            if data['delist'] == 'tt':

...
class TestHrHolidaysBase(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.user_hruser_id = Users.create({
            ...
            'groups_id': [(6, 0, [group_employee_id, self.ref('hr_holidays_community.group_hr_holidays_user')])]
        }).id
        self.user_hrmanager_id = Users.create({
            'name': 'Bastien HrManager',
            'login': 'bastien',
            'email': 'bastien.hrmanager@example.com',
            'groups_id': [(6, 0, [group_employee_id, self.ref('hr_holidays_community.group_hr_holidays_manager')])]
        }).id

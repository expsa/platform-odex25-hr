...
class HolidaysType(models.Model):
    ...
    categ_id = fields.Many2one('calendar.event.type', string='Meeting Type',
                               help='Once a leave is validated, Odoo will'
                                    'create a corresponding meeting of this type in the calendar.')
    ...
    active = fields.Boolean('Active', default=True,
                            help="If the active field is set to false, it will allow you to hide the leave type "
                                 "without removing it.")
    ...
    double_validation = fields.Boolean(string='Apply Double Validation',
                                       help="When selected, the Allocation/Leave Requests for this type require a "
...
class Holidays(models.Model):
    ...
    number_of_days = fields.Float('Number of Days', compute='_compute_number_of_days', tracking=True)
    ...
    def _compute_can_reset(self):
        ...
        """
        user = self.env.user
        group_hr_manager = self.env.ref('hr_holidays_community.group_hr_holidays_manager')
        for holiday in self:
    ...
    def _get_number_of_days(self, date_from, date_to, employee_id):
        ...
        if employee_id:
            ...
            return employee._get_work_days_data(from_dt, to_dt, calendar=None)['days']
    ...
    def _check_state_access_right(self, vals):
        if vals.get('state') and vals['state'] not in ['draft', 'confirm', 'cancel'] and not self.env[
            'res.users'].has_group('hr_holidays_community.group_hr_holidays_user'):
            return False
        return True
    ...
    def _check_security_action_approve(self):
        if not self.env.user.has_group('hr_holidays_community.group_hr_holidays_user'):
    ...
    def action_validate(self):
        ...
        for holiday in self:
            ...
            if holiday.state == 'validate1' and not holiday.env.user.has_group(
                    'hr_holidays_community.group_hr_holidays_manager'):
    ...
    def _prepare_holidays_meeting_values(self):
        self.ensure_one()
        meeting_values = {
            'name': _("%s on Time Off : %.2f day(s)") % (
            self.employee_id.name or self.category_id.name, self.number_of_days_temp),
            'categ_ids': [(6, 0, [
    ...
    def _track_subtype(self, init_values):
        if 'state' in init_values and self.state == 'validate':
            return self.env.ref('hr_holidays_community.mt_holidays_approved')
        elif 'state' in init_values and self.state == 'validate1':
            return self.env.ref('hr_holidays_community.mt_holidays_first_validated')
        elif 'state' in init_values and self.state == 'confirm':
            return self.env.ref('hr_holidays_community.mt_holidays_confirmed')
        elif 'state' in init_values and self.state == 'refuse':
            return self.env.ref('hr_holidays_community.mt_holidays_refused')
    ...
    def _notification_recipients(self, message, groups):
        ...
        new_group = (
            'group_hr_holidays_user', lambda partner: bool(partner.user_ids) and any(
                user.has_group('hr_holidays_community.group_hr_holidays_user') for user in partner.user_ids), {
                'actions': hr_actions,
            })

...
class Employee(models.Model):
    ...
    remaining_leaves = fields.Float(compute='_compute_remaining_leaves', string='Remaining Legal Leaves',
                                    ...
                                    help='Total number of legal leaves allocated to this employee, change this value '
                                         'to create allocation/leave request. '
    ...
    current_leave_id = fields.Many2one('hr.holidays.status', compute='_compute_leave_status',
    ...
    is_absent_today = fields.Boolean('Absent Today', compute='_compute_absent_employee',
    ...
    def _compute_show_leaves(self):
        show_leaves = self.env['res.users'].has_group('hr_holidays_community.group_hr_holidays_user')
        for employee in self:
            if show_leaves :
    ...
    def _compute_absent_employee(self):
        ...
        for employee in self:
            employee.is_absent_today = result[employee.id]

...
def update_workflow(self):
    ...
    self.env.cr.commit()
    for rec in wkf_obj.search([]):
        wkf_model = self.env[rec.model_id.model]
        wkf = self.env['odoo.workflow.model'].with_context({'model': rec.model_id.model})
        if wkf_model._fields.get('state'):
            wkf_model._fields.get('state').selection = wkf._load_state()
            wkf_model._fields.get('state').default = lambda wkf: wkf._compute_default_state()
        for node in rec.node_ids + rec.deactived_node_ids:
            node.active = node.pre_active
...
def inherit_workflow_manager(cr, model):
    ...
    if isinstance(model_name, str):
        # Check model
        if not re.match(EXCLUDED_MODELS, model_name) and not is_transient and not is_abstract:
                    ...
                    if rec.get('model', False) == model_name:
                        ...
                        if rec.get('mail_thread_add', False):
                            ...
                            if 'mail.activity.mixin' not in parents:
                                parents.append('mail.activity.mixin')
                            if 'portal.mixin' not in parents:
                                parents.append('portal.mixin')
    return parents

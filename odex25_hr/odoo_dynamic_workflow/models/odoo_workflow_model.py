...
from odoo.addons.base.models.ir_ui_view import transfer_field_to_modifiers, transfer_node_to_modifiers, \
...
def setup_modifiers(node, field=None):
...
class OdooWorkflowModel(models.Model):
    ...
    def _compute_default_state(self):
        ...
        rec_model = self._context.get('model') or self._name
        # Get Flow Start
        wkf_recs = wkf_obj.with_context(active_test=False).search([('model_id', '=', rec_model)])
        for wkf_rec in wkf_recs:
            if wkf_rec.active:
                ...
                return wkf_rec.model_state_default
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        # Code Start
        res = super(OdooWorkflowModel, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,
                                                             submenu=submenu)
    ...
    def _remove_default_header(self, view_type, res):
        ...
        wkf_rec = self.env['odoo.workflow'].search([('model_id', '=', self._name)])
        # Validation
        if not wkf_rec.remove_default_header:
            return res
                ...
                for button in node.button_ids:
                    ...
                    if button.btn_hide:
                        if not _check_condition_code(button):
                            continue
                    if res['view_id'] not in button.view_ids.ids:
                        continue
                    # Add Button to View
                    ...
                    btn_exec_action_el.set('custom_btn', '1')
                    ...
                    if button.group_ids:
                        has_group = False
                        btn_exec_action_el.set('groups', ','.join(button.group_ids.get_xml_id().values()))
                        user_rec = self.env['res.users'].browse(self._uid)
                        for ext_id in button.group_ids.get_xml_id().values():
                            if user_rec.has_group(ext_id):
                                has_group = True
                                break
                        if not has_group:
                            btn_exec_action_el.set('invisible', "1")
    ...
    def _load_state_view(self, view_type, res):
        ...
        if wkf_rec and view_type == 'form':
            ...
            if header_el is not False:
                ...
                for node in wkf_rec.node_ids:
                    # Read Visible States
                    if node.is_visible and res['view_id'] in node.view_ids.ids:
    ...
    def _add_mail_thread_view(self, view_type, res):
        ...
        if view_type == 'form' and wkf_rec.mail_thread_add:
            ...
            if wkf_rec.activities_add:
                activities_el = etree.Element('field', attrib={'name': 'activity_ids', 'widget': 'mail_activity'})
                _add_field_def_to_view(res, 'activity_ids', activities_el)
                mail_thread_root_el.append(activities_el)
            if wkf_rec.followers_add:
                followers_el = etree.Element('field',
                                             attrib={'name': 'message_follower_ids', 'widget': 'mail_followers'})

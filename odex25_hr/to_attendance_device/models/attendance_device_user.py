...
class AttendanceDeviceUser(models.Model):
    ...
    uid = fields.Integer(string='UID', help='The ID (technical field) of the user/employee in the device storage',
                         readonly=True)
    user_id = fields.Char(string='ID Number', size=8, help='The ID Number of the user/employee in the device storage',
                          required=True, )
    ...
    not_in_device = fields.Boolean(string='Not in Device', readonly=True,
                                   help="Technical field to indicate this user is not available in device storage."
    ...
    def _compute_current_attendance(self):
        for r in self:
            r.attendance_id = self.env['user.attendance'].search([('user_id', '=', r.id)], limit=1,
                                                                 order='timestamp DESC') or False
        ...
        for r in self:
            if r.device_id and r.device_id.unique_uid:
                duplicate = self.search(
                    [('id', '!=', r.id), ('device_id', '=', r.device_id.id), ('user_id', '=', r.user_id)], limit=1)
                if duplicate:

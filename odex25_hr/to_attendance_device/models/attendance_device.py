...
class AttendanceDevice(models.Model):
    ...
    firmware_version = fields.Char(string='Firmware Version', readonly=True,
                                   help="The firmware version of the device which will be filled automatically when "
                                        "you hit the 'Get Device Info' button.")
    serialnumber = fields.Char(string='Serial Number', readonly=True,
                               help="The serial number of the device which will be filled automatically when you hit "
                                    "the 'Get Device Info' button.")
    oem_vendor = fields.Char(string='OEM Vendor', readonly=True,
                             help="The OEM Vendor of the device which will be filled automatically when you hit the "
                                  "'Get Device Info' button.")
    platform = fields.Char(string='Platform', readonly=True,
                           help="The Platform of the device which will be filled automatically when you hit the "
                                "'Get Device Info' button.")
    fingerprint_algorithm = fields.Char(string='Fingerprint Algorithm', readonly=True,
                                        help="The Fingerprint Algorithm (aka ZKFPVersion) of the device which will be "
                                             "filled automatically when you hit the 'Get Device Info' button.")
    device_name = fields.Char(string='Device Name', readonly=True,
                              help="The model of the device which will be filled automatically when you hit the "
                                   "'Get Device Info' button.")

    work_code = fields.Char(string='Work Code', readonly=True,
                            help="The Work Code of the device which will be filled automatically when you hit the"
                                 " 'Get Device Info' button.")
    ...
    ignore_unknown_code = fields.Boolean(string='Ignore Unknown Code', default=False,
                                         help='Sometimes you don\'t want to load attendance data with status '
                                              'codes those not declared in the table below. In such the case, '
                                              'check this field.',
                                         readonly=True, states={'draft': [('readonly', False)]})
    ...
    auto_clear_attendance = fields.Boolean(string='Auto Clear Attendance Data', default=False,
                                           readonly=True, states={'draft': [('readonly', False)]},
                                           help='Check this to clear all device attendance data after download into'
                                                ' Odoo')
    ...
    auto_clear_attendance_hour = fields.Float(string='Auto Clear At', required=True,
                                              default=0.0,
                                              help="The time (in the attendance device's timezone) to clear attendance "
                                                   "data after download.")
    ...
    unique_uid = fields.Boolean(string='Unique UID', default=True, required=True,
                                readonly=True, states={'draft': [('readonly', False)]},
                                help='Some Bad Devices allow uid duplication. In this case, uncheck this field. But it '
                                     'is recommended to change your device.')
    last_attendance_download = fields.Datetime(string='Last Sync.', readonly=True,
                                               help='The last time that the attendance data was downloaded from the '
                                                    'device into Odoo.')

    map_before_dl = fields.Boolean(string='Map Employee Before Download', default=True,
                                   help='Always try to map users and employees (if any new found) before downloading '
                                        'attendance data.')
    create_employee_during_mapping = fields.Boolean(string='Create Employees During Mapping', default=False,
                                                    help="If checked, during mapping between Device's Users and "
                                                         "company's employees, unmapped device"
                                                         " users will try to create a new employee then map "
                                                         "accordingly.")
    ...
    protocol = fields.Selection([('udp', 'UDP'), ('tcp', 'TCP')], string='Protocol', required=True, default='tcp',
                                help="Some old devices do not support TCP. In such the case, please try on switching"
                                     " to UDP.")
    ...
    unaccent_user_name = fields.Boolean(string='Unaccent User Name', default=True,
                                        help="Some Devices support Unicode names such as the ZKTeco K50, some others do"
                                             " not. In addition to this, the name field on devices is usually limited "
                                             "at about 24 Latin characters or less Unicode characters. "
                                             "Unaccent is sometimes a workaround for long Unicode names")
    ...
    def onchange_unique_uid(self):
        if not self.unique_uid:
            message = _('This is for experiment to check if the device contains bad data with non-unique user\'s uid.'
                        ...
                        'NOTE:\n'
                        '- non-latin user_id are not suportted.\n'

...
class EmployeeUploadWizard(models.TransientModel):
    ...
    employee_ids = fields.Many2many('hr.employee', string='Employees to upload', default=_get_employee_ids,
                                    required=True)
        ...
        for line in self.line_ids:
            if not line.employee_id.barcode:
                raise ValidationError(
                    _('Employee %s has no Badge ID (employee barcode) set. Please set it on the employee profile.')

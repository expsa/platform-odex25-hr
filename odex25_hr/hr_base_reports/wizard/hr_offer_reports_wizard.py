...
class HrOfferReportsWizard(models.TransientModel):
...
class ReportHrOfferJobOfEmployee(models.AbstractModel):
    ...
    _description = 'Offer Job Template'

    @api.model
    def _get_report_values(self, docids, data=None):
        date_start = datetime.strptime(str(data['form']['date_start']), DATETIME_FORMAT)
        date_end = datetime.strptime(str(data['form']['date_end']), DATETIME_FORMAT) + timedelta(days=1)
        department = data['form']['department_id']
        job = data['form']['job_id']

        docs = []

        job_offer = self.env['hr.job.offer.signature'].search([
...
class HrJobOfferParseXlsx(models.AbstractModel):
    ...
    _description = "XLSX Action Job Offer Report"

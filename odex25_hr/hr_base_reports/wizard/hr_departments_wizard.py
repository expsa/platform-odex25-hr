...
class HrDepartmentJobWizard(models.TransientModel):
    _name = 'hr.department.job.wizard'
    _description = 'Department Job Wizard'
...
class ReportDepartmentJobParse(models.AbstractModel):
    ...
    _description = 'Department Job Report View'
...
class HrDepartmentJobParseXlsx(models.AbstractModel):
    ...
    _description = "XLSX Action Department Report"
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        ...
        if type == 'departments':
            ...
            sheet.merge_range('B5:F5', _('Jobs Of Employees'), format2)
            sheet.write(6, 2, _('jobs'), format2)
            sheet.set_column('C:C', 25)
            sheet.write(6, 3, _(u'Total'), format2)
            sheet.set_column('D:H', 25)
            row = 6
            for line in docs:
                row += 1
